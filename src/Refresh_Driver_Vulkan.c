...
static void VULKAN_BindComputeBuffers(
	...
	for (i = 0; i < computePipeline->pipelineLayout->bufferDescriptorSetCache->bindingCount; i += 1)
	{
		...
		VULKAN_INTERNAL_PrepareBufferForWrite(
			...
			vulkanCommandBuffer,
			bufferContainer,
			pBindings[i].writeOption == REFRESH_COMPUTEBINDOPTIONS_CYCLE ? VULKAN_RESOURCEUPDATEOPTION_CYCLE : VULKAN_RESOURCEUPDATEOPTION_SAFE,
			RESOURCE_ACCESS_COMPUTE_SHADER_BUFFER_READ_WRITE
		);
...
static void VULKAN_UploadToBuffer(
	...
	VulkanBuffer *vulkanBuffer = VULKAN_INTERNAL_PrepareBufferForWrite(
		...
		vulkanCommandBuffer,
		gpuBufferContainer,
		writeOption == REFRESH_BUFFERWRITEOPTIONS_CYCLE ? VULKAN_RESOURCEUPDATEOPTION_CYCLE : VULKAN_RESOURCEUPDATEOPTION_UNSAFE,
		RESOURCE_ACCESS_TRANSFER_WRITE
	);
